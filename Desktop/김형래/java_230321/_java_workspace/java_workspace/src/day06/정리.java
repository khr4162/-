...
public class 정리 {
	...
	public static void main(String[] args) {
		 ...
		 *  - 문자형 : char(정수형으로 분류하기도 함. 유니코드 표에 코드로 저장) : 1글자 
		 *  - boolean (true/false)
		 * - 참조형 변수 : 객체의 주소를 저장하는 변수(객체명) : new 키워드를 통해 객체를 생성
		 * - 자료형 변환 : 변수, 상수, 리터럴 값들을 일시적으로 다른 자료형으로 변환
		 *  - 자동형변환 : 정수 -> 실수, 작은 바이트 -> 큰 바이트 (작은 거에서 큰걸로 갈때)
		 *  - 명시적형변환 : 자동으로 형변환이 되지 않는 경우 - (자료형)변수명
		 *  
		 * 2. 연산자
		 * - 대입연산자(=)
		 *  - 오른쪽에 있는 값을 왼쪽 변수에 저장
		 *  - 왼쪽에는 반드시 저장 가능한 변수/객체 필수
		 ...
		 *  -switch : 값이 나오는 식, 변수
		 *   - 하나의 case가 끝나면 break;를 걸어줘야 함
		 *   - 생략하면 하위 모든 case가 실행됨.
		 * 
		 * 4. 반복문 : 규칙성이 있는 반복
		 *  - for, while, dowhile문 : 조건식은 true 조건으로 작성.
		 *  - for(초기화; 조건식; 증감식){
		 *  실행문;}
		 *  
		 *  - while(조건식;){
