...
public class BaseBallGame {
	...
	public static void main(String[] args) {
		 ...
		 * 리턴타입 => 반드시 하나(자료형, 배열 등)
		 * 
		 * 랜덤번호 생성(배열에 추가)
		 * 스트라이크 메서드, 볼 메서드, 출력 메서드
		 */
		int com[] = new int[3];
		int user[] = new int[3];
		int max = 9, min = 1; //랜덤수의 범위
		int s=0, b=0;
		int count=0;
		...
		while(s!=3) {
			//사용자번호 입력받기
			System.out.println("1~9까지 숫자 3개를 입력하세요");
	...
	//매개변수 => 갯수, 시작값 (int)(math.random()*갯수)+시작
...
//	
	...
	public static int random(int max, int min) {
		if(max < min) { //max와 min의 값이 바뀌어서 들어올때 처리
			int tmp =max;
			max = min;
			min = tmp;
		}
	...
	//배열 생성 메서드
	//true가 리턴 되면 정상적으로 배열이 생성됨을 알림
	public static boolean randomArray(int arr[], int max, int min) {
		if(arr==null) {
			return false;
		}
		if(max-min+1<arr.length) { //랜덤수의 갯수가 배열의 length보다 작다면
			return false;
		}
		int i=0;
		while(i<arr.length) {
